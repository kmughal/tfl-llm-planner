  ...
  return <motion.div className="fixed inset-0 z-[70] flex flex-col overflow-hidden bg-[#f5f5f7] text-[#161617]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    ...
    <main className="flex-1 overflow-y-auto">
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-5 py-6 sm:px-8">
        ...
        <section className="rounded-3xl border border-black/[.06] bg-white p-5 shadow-[0_10px_35px_rgba(0,0,0,.045)] sm:p-6"><SectionTitle icon={<TrainFront size={17} />} title="Every line" subtitle="Select a line for complete live detail" count={lines.length} />{data?.errors.lines && !lines.length ? <Unavailable label="Line status" /> : <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">{lines.map((line,index) => <motion.button type="button" key={line.id} onClick={() => setSelectedLine(selectedLine === line.id ? null : line.id)} initial={{ opacity:0,y:10 }} animate={{ opacity:1,y:0 }} transition={{ delay:Math.min(index*.025,.35) }} whileHover={{ y:-2 }} className="relative min-h-24 overflow-hidden rounded-2xl border border-black/[.055] bg-[#fafafa] p-3 text-left"><span className="absolute inset-y-0 left-0 w-1.5" style={{ background:color(line.id) }} /><div className="flex items-start justify-between gap-2 pl-1"><span className="text-xs font-bold">{line.name}</span><span className="mt-0.5 h-2 w-2 rounded-full" style={{ background:goodLine(line)?"#30d158":"#ff9f0a" }} /></div><div className="mt-4 pl-1 text-[10px] font-semibold text-[#6e6e73]">{statusOf(line).statusSeverityDescription}</div></motion.button>)}</div>}<AnimatePresence>{activeLine && <motion.div initial={{ opacity:0,height:0 }} animate={{ opacity:1,height:"auto" }} exit={{ opacity:0,height:0 }} className="overflow-hidden"><div className="mt-4 flex items-center gap-4 rounded-2xl bg-[#f5f5f7] p-4"><span className="h-12 w-1.5 rounded-full" style={{ background:color(activeLine.id) }} /><div><div className="text-sm font-bold">{activeLine.name}</div><p className="mt-1 text-xs leading-5 text-[#6e6e73]">{statusOf(activeLine).reason || statusOf(activeLine).statusSeverityDescription}</p></div></div></motion.div>}</AnimatePresence></section>
