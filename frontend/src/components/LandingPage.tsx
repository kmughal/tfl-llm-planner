...
const NETWORKS: {
...
}[] = [
  {
    ...
    CardBg: EuroCardBg,
    examples: [
      { text: "Show live map of all Eurostar trains now",      tool: "get_eurostar_live_map" },
      { text: "Where are Eurostar trains right now?",          tool: "get_eurostar_live_map" },
      ...
      { text: "What trains run London → Paris today?",         tool: "get_euromap_plans" },
      { text: "Last train from Paris to London tonight?",      tool: "get_euromap_plans" },
      { text: "All Eurostar services Brussels → London",       tool: "get_euromap_plans" },
      { text: "Stop times for service {number} on {date}",     tool: "get_euromap_plan_by_id",             template: true },
      { text: "Technical plan for train {number} on {date}",   tool: "get_euromap_technical_plan_by_id",   template: true },
      { text: "All technical plans for today",                 tool: "get_euromap_technical_plans" },
      { text: "How busy is Eurostar service 9005 today?",      tool: "get_traveler_summary" },
      { text: "Passenger load for all services on {date}",     tool: "get_traveler_summary",               template: true },
      { text: "Class breakdown for service {number}",          tool: "get_traveler_summary",               template: true },
    ],
  },
  {
    key: "SNCF",
    ...
    examples: [
      ...
      { text: "Arrivals board at Lyon Part-Dieu",          tool: "get_sncf_arrivals" },
  ...
  },
  {
    ...
    examples: [
      ...
      { text: "Status of all tube lines right now",        tool: "get_status_by_mode" },
      { text: "Any delays on the Underground today?",      tool: "get_status_by_mode" },
