...
func TestNormalizeAliasesAndArguments(t *testing.T) {
	tests := []struct {
		...
		{"plan by id defaults date", call("get_euromap_plan_by_id", `{}`), "Show me full stop times for Eurostar service 9435 today", "get_euromap_plan_by_id", "date", time.Now().UTC().Format("20060102")},
		{"plan by id schema date fallback", call("get_euromap_plan_by_id", `{"serviceCode":"9435","date":{"type":"string","description":"Date portion in YYYYMMDD format"}}`), "Show me full stop times for Eurostar service 9435 today", "get_euromap_plan_by_id", "date", time.Now().UTC().Format("20060102")},
		{"plan by id unwraps nested parameters", call("get_euromap_plan_by_id", `{"name":"get_euromap_plan_by_id","parameters":{"serviceCode":"9435","date":{"type":"string","description":"Date portion in YYYYMMDD format"}}}`), "Show me full stop times for Eurostar service 9435 today", "get_euromap_plan_by_id", "serviceCode", "9435"},
